...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  useEffect(() => {
    if (generationMode === GenerationMode.REFERENCES_TO_VIDEO) {
      ...
      // Removed setAspectRatio(AspectRatio.LANDSCAPE) to allow user choice, 
      // but defaulting is handled by the component state or manual change.
  ...
  const handleSelectMode = (mode: GenerationMode) => {
    ...
    // When switching to reference mode, LANDSCAPE is a safer default per guidelines
    ...
    if (generationMode === GenerationMode.REFERENCES_TO_VIDEO) {
      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center justify-center gap-4">
          ...
          <div className="flex items-center gap-4 py-2 border-t border-gray-700 w-full justify-center">
            <span className="text-xs font-medium text-gray-400">Target Aspect Ratio:</span>
            <div className="flex bg-[#1f1f1f] p-1 rounded-lg border border-gray-600">
              <button
                type="button"
                onClick={() => setAspectRatio(AspectRatio.LANDSCAPE)}
                className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all flex items-center gap-2 ${aspectRatio === AspectRatio.LANDSCAPE ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-400 hover:text-gray-200'}`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all flex items-center gap-2 ${aspectRatio === AspectRatio.PORTRAIT ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-400 hover:text-gray-200'}`}
              >
                <div className="w-2.5 h-3.5 border border-current rounded-sm rotate-90" />
                9:16
              </button>
            </div>
          </div>
